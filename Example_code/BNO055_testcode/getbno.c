...
int main(int argc, char *argv[])
{
    ...
    * ----------------------------------------------------------- */
    ...
    if (verbose == 1)
        printf("Debug: ts=[%lld] date=%s", (long long)tsnow, ctime(&tsnow));
    ...
    * ----------------------------------------------------------- */
    ...
    {
        opmode_t newmode;
        if (strcmp(opr_mode, "config") == 0)
            newmode = config;
        else if (strcmp(opr_mode, "acconly") == 0)
            newmode = acconly;
        else if (strcmp(opr_mode, "magonly") == 0)
            newmode = magonly;
        else if (strcmp(opr_mode, "gyronly") == 0)
            newmode = gyronly;
        else if (strcmp(opr_mode, "accmag") == 0)
            newmode = accmag;
        else if (strcmp(opr_mode, "accgyro") == 0)
            newmode = accgyro;
        else if (strcmp(opr_mode, "maggyro") == 0)
            newmode = maggyro;
        else if (strcmp(opr_mode, "amg") == 0)
            newmode = amg;
        else if (strcmp(opr_mode, "imu") == 0)
            newmode = imu;
        else if (strcmp(opr_mode, "compass") == 0)
            newmode = compass;
        else if (strcmp(opr_mode, "m4g") == 0)
            newmode = m4g;
        else if (strcmp(opr_mode, "ndof") == 0)
            newmode = ndof;
        else if (strcmp(opr_mode, "ndof_fmc") == 0)
            newmode = ndof_fmc;
        else
        {
    ...
    * ----------------------------------------------------------- */
    ...
    {
        power_t newmode;
        if (strcmp(pwr_mode, "normal") == 0)
            newmode = normal;
        else if (strcmp(pwr_mode, "low") == 0)
            newmode = low;
        else if (strcmp(pwr_mode, "suspend") == 0)
            newmode = suspend;
        else
        {
        ...
        {
            if (verbose == 1)
                printf("Debug: Sensor already in mode %s [0x%02X].\n", pwr_mode, newmode);
            exit(0);
        }
    ...
    if (argflag == 4)
    {
       ...
       * -------------------------------------------------------- */
        if (bnoc.scal_st == 3)
            ...
            printf("Error: Sensor not fully calibrated, abort writing to file %s.\n", calfile);
    }
    ...
    * ----------------------------------------------------------- */
    if (strcmp(datatype, "inf") == 0)
    {
       ...
       * ----------------------------------------------------------- */
        ...
        print_mode(bnoi.opr_mode);
        printf("        Power Mode = ");
        print_power(bnoi.pwr_mode);
        printf("Axis Configuration = ");
        print_remap_conf(bnoi.axr_conf);
        printf("   Axis Remap Sign = ");
        print_remap_sign(bnoi.axr_sign);
        printf("System Status Code = ");
        print_sstat(bnoi.sys_stat);
        printf("System Clocksource = ");
        print_clksrc();

        printf("Accelerometer Test = ");
        if ((bnoi.selftest >> 0) & 0x01)
            printf("OK\n");
        else
            printf("FAIL\n");

        printf(" Magnetometer Test = ");
        if ((bnoi.selftest >> 1) & 0x01)
            printf("OK\n");
        else
            printf("FAIL\n");

        printf("    Gyroscope Test = ");
        if ((bnoi.selftest >> 2) & 0x01)
            printf("OK\n");
        else
            printf("FAIL\n");

        printf("MCU Cortex M0 Test = ");
        if ((bnoi.selftest >> 3) & 0x01)
            printf("OK\n");
        else
            printf("FAIL\n");
        ...
        if (bnoi.opr_mode > 0)
        {
            if ((bnoi.unitsel >> 4) & 0x01)
                printf("%d°F\n", bnoi.temp_val);
            else
                printf("%d°C\n", bnoi.temp_val);
        }
        else
            printf("no data in CONFIG mode\n");

        printf("\n----------------------------------------------\n");
        struct bnoaconf bnoac;
        if (get_acc_conf(&bnoac) == 0)
